...
from qwen3.layers import ParallelTransformersBlock
...
class ParallelQwen3Model(nn.Module):
    def __init__(self, config):
        ...
        self.rope = RoPE(config.head_dim, config.rope_base, config.context_length)
        self.layers = nn.ModuleList([
            ParallelTransformersBlock(config)
    ...
    def forward(self, token_ids, position_ids, kv_cache=None, attn_mask=None):
        ...
        new_kv_cache = []
        for layer_idx, attn_block in enumerate(self.layers):
            layer_kv_cache = kv_cache[layer_idx] if kv_cache is not None else None
            x, new_layer_kv_cache = attn_block(x, self.rope, position_ids, layer_kv_cache, attn_mask)
