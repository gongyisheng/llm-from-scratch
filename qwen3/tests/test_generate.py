...
def test_batch(model_name):
    """Batch generation with left padding should match single-sequence generation."""
    if not checkpoint_available(model_name):
        pytest.skip(f"Checkpoint {model_name} not downloaded")
    ...
    # --- single-sequence outputs (ground truth) ---
    single_outputs = []
    for prompt in prompts:
        ...
        single_outputs.append(output_ids)
    ...
    for i, prompt in enumerate(prompts):
        assert batch_outputs[i] == single_outputs[i], (
            f"Prompt {i} mismatch:\n"
            f"  single: {tokenizer.decode(single_outputs[i])}\n"
            f"  batch:  {tokenizer.decode(batch_outputs[i])}"
        )
